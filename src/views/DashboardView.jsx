...
const DashboardView = ({ data = [], loading, onRefresh, onNavigate }) => {
  ...
  const dashboardData = useMemo(() => {
    ...
    data.forEach(ticket => {
        // -----------------------------------------------------------------------
        // FILTRO: Ignorar "Apertura de Expediente" (Dummy) para el Dashboard
        // Criterio: Rol Clasificación + ID 1 + Monto 0
        // -----------------------------------------------------------------------
        if (ticket.tipo_operacion === 'clasificacion') {
          const esDummy = ticket.concepto == 1 && parseFloat(ticket.importe || 0) === 0;
          if (esDummy) return; // Saltamos este ticket, no afecta contadores ni gráficas
        }
        // -----------------------------------------------------------------------
