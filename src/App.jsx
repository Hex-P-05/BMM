...
const CaptureForm = ({ onSave, onCancel, existingData, role, userName }) => {
  ...
  // 1. Auto-cálculo del monto total
  useEffect(() => {
    const sum = 
  ...
  // 2. LÓGICA DEL CONSECUTIVO Y COMENTARIOS
  useEffect(() => {
    let nextNum = 0;
    
    // Solo calculamos si hay un prefijo de 3 letras
    if (formData.prefijo && formData.prefijo.length === 3) {
        // Contamos cuántos registros existen con este MISMO prefijo
    ...
    // CONCATENACIÓN: CONCEPTO + PREFIJO + CONSECUTIVO + CONTENEDOR
  ...
  const handleChange = (e) => {
    ...
    // Lógica Prefijo (Max 3 chars, Mayúsculas)
    if (name === 'prefijo') {
        if (value.length <= 3) setFormData({ ...formData, [name]: value.toUpperCase() });
        return;
    }

    // Lógica Proveedor (Auto-llenado Bancos)
    if (name === 'proveedor') {
      ...
      <div className="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          ...
          <div className="bg-blue-50/50 p-5 rounded-xl border border-blue-100">
             ...
             {/* VISUALIZADOR DE LÓGICA CONSECUTIVA */}
...
const ListView = ({ data, onPayItem, onPayAll, onCloseOperation, role, onEdit }) => {
  ...
  // Solo permitimos seleccionar items que estén CERRADOS
  const closedItems = filteredData.filter(item => item.status === 'closed');

  const handleSelectAll = (e) => {
    if (e.target.checked) {
        // Seleccionar todos los visibles que estén cerrados
  ...
  const handleBulkDownload = () => {
    // Filtramos los datos completos para obtener los objetos seleccionados
    ...
    if (itemsToPrint.length > 0) {
        ...
        setSelectedIds([]); // Limpiar selección tras descargar
    }
  };
    ...
    <div className="space-y-4 animate-fade-in h-full flex flex-col">
      <div className="flex flex-col md:flex-row justify-between items-center bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex-shrink-0 gap-4">
        <div className="flex items-center gap-4">
            ...
            {/* BOTÓN DE DESCARGA MASIVA (Solo aparece si hay seleccionados) */}
                ...
                <tr className="bg-slate-50 border-b border-slate-200 text-xs font-bold text-slate-500 uppercase h-12">
                    ...
                    <th className="p-4 w-10 sticky left-0 z-50 bg-slate-50 border-r text-center">
                        <input 
                            ...
                            title="Seleccionar todos los cerrados"
                        />
                    </th>
                    ...
                    <tr className={`hover:bg-slate-50 border-b border-slate-100 transition-colors ${expandedRow === item.id ? 'bg-blue-50/30' : ''}`}>
                        ...
                        <td className="p-4 text-center sticky left-0 z-20 bg-white border-r border-slate-100">
                            <input 
                                ...
                                disabled={!isClosed} // Solo habilitado si está cerrado
                                className={`w-4 h-4 rounded ${isClosed ? 'cursor-pointer accent-blue-600' : 'cursor-not-allowed bg-slate-100'}`}
                            />
                        </td>
                        ...
                        <td className="p-4 text-center">
                             <button
                                ...
                                title={isClosed ? "Descargar PDF" : "Operación no cerrada"}
                             >
                                <FileText size={18} />
                             </button>
                        </td>
